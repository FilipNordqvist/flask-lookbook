...
class TestContact:
    ...
    def test_send_email_missing_fields(self, client):
        ...
        response = client.post(
            "/send", data={"message": "Test message"}, follow_redirects=True
        )
        assert response.status_code == 200

        # Missing message
        response = client.post(
            "/send", data={"email": "test@example.com"}, follow_redirects=True
        )
        assert response.status_code == 200
    ...
    def test_send_email_html_escaping(self, client):
        ...
        with patch("routes.contact.resend.Emails.send") as mock_send:
            ...
            assert (
                "<script>" not in html_content
            ), "HTML should be escaped - raw script tag should not be present"
            assert (
                "&lt;script&gt;" in html_content
            ), "HTML should be escaped - escaped script tag should be present"
    ...
    def test_send_email_reply_to_uses_safe_email(self, client):
        ...
        with patch("routes.contact.resend.Emails.send") as mock_send:
            ...
            assert (
                reply_to == test_email
            )  # Should be the escaped version (same for normal emails)
